...
HELIDON_APP_URL='http://localhost:8081'
...
sed -i -E "s/(\s+client-secret: )\S*/\1${CLIENT_SEC}/" ./03_mp-jwt/castle/src/main/resources/application.yaml
sed -i -E "s/(\s+client-secret: )\S*/\1${CLIENT_SEC}/" ./03_mp-jwt/watchtower/src/main/resources/application.yaml
